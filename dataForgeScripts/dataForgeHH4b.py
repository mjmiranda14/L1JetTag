...
if (
    len(sys.argv) != 5
    or int(sys.argv[3]) < 0
    or (int(sys.argv[4]) != 0 and int(sys.argv[4]) != 1)
):
    print(
        "USAGE: <input file> <date/file tag> <pT cut> <candidates (0 for PF, 1 for PUPPI)>"
    )
    sys.exit(1)
...
A1 = ROOT.TH1F(
    "No. of matched b jets", "Number of matched b jets in each event", 5, 0, 5
)
...
for entryNum in range(eventNum):
    ...
    bannedbQuarks = (
        []
    )  # Same deal but with indices within the gen tree corresponding to b quarks
    for i in range(len(obj)):
        ...
        if (
        ...
        ):  # obj[i][1] in chargedHadrons and (i not in bannedParts):
            tempTLV = obj[i][0]
            ...
            if obj[i][1] == 22 or obj[i][1] == 130:
                ...
                )  # Add in dZ, dX, dY, Particle Pt, Eta, & Phi, last 3 features to be scaled later			jetPartList.extend(seedParticle)
